    ...
    pub fn run(&mut self, key: usize, redraw: &mut bool) -> bool {
        ...
        match instruction {
            ...
            [0x0, 0x0, 0xC, c] => {
                ...
                self.screen.copy_from_slice(&new_screen);
            }
            // 00E0 - Clears the screen.
            [0x0, 0x0, 0xE, 0x0] => {
                self.screen = vec![0; SCHIP8_NUM_PIXELS];
                *redraw = true;
            }
            ...
            [0xC, x, b, c] => {
                let nn = ((b << 4) | c) as u16;
                let rand = (Math::random() * (0xFF as f64)) as u16;
...
#[wasm_bindgen]
pub fn pass_schip8_emulator() -> JsValue {
    let tetris: Vec<u8> = vec![
    ...
    serde_wasm_bindgen::to_value(&schip8).unwrap()
}
